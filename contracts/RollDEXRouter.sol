...
import "./RollDEXSwapReentrancyGuard.sol";

/// @title RollDEX Router
/// @notice Router for stateless execution of swaps against multiple DEX protocols
contract RollDEXRouter is IRollDEXRouter, Ownable, RollDEXSwapReentrancyGuard {
    IWETH public WETH;
    ...
    constructor(address _weth, address _treasury) {
        require(_weth != address(0), "RollDEX: Wrong WETH address");
        require(_treasury != address(0), "RollDEX: Wrong treasury address");
    ...
    receive() external payable {
        require(_msgSender() == payable(address(WETH)), "RollDEX: Forbidden token transfer");
        // only accept native chain tokens via fallback from the wrapper contract
    }

    // @inheritdoc IRollDEXRouter
    function swap(
        ...
        for (uint256 i = 0; i < request.to.length; i++) {
            ...
            if (amountOut > request.amountOutExpected[i]) {
                ...
                require(
                    ...
                    "RollDEX: Insufficient output amount"
                );
    ...
    function executeSwap(address from, uint256 amountIn, Swap calldata swapData) private returns (uint256) {
        if (swapData.family == UNISWAP_V3) {
            ...
            revert("RollDEX: Unknown DEX family");
        }
    }
